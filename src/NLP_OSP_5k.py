...
## Feature selection, rename, and encoding
new_df = df.copy()
...
features = pd.concat([xform_data.reset_index(drop=True), new_df[feature_list+category_columns].reset_index(drop = True)], axis=1)
...
thresholds = {'analyze': 0.5, 'apply': 0.5, 'create': 0.5,
              'evaluate': 0.3, 'remember': 0.3, 'understand': 0.3}


# Map class names to their indices
class_indices = {class_name: index for index, class_name in enumerate(clf.classes_)}

# Apply the thresholds
y_pred_adjusted = np.array([clf.classes_[np.argmax(row)] for row in y_probs])

for class_name, threshold in thresholds.items():
    class_index = class_indices[class_name]
    high_prob_indices = (y_probs[:, class_index] >= threshold)
    y_pred_adjusted[high_prob_indices] = class_name

print("Adjusted Accuracy:", accuracy_score(y_test, y_pred_adjusted))
print("Adjusted Precision:", precision_score(y_test, y_pred_adjusted, average='weighted', zero_division=0))
print("Adjusted Recall:", recall_score(y_test, y_pred_adjusted, average='weighted'))
print("Adjusted F1 Score:", f1_score(y_test, y_pred_adjusted, average='weighted', zero_division=0))
print(classification_report(y_test, y_pred_adjusted))

# Adjusted Confusion Matrix
adjusted_conf_matrix = confusion_matrix(y_test, y_pred_adjusted)
sns.heatmap(adjusted_conf_matrix, annot=True, fmt='g')
...
full_probs = clf.predict_proba(features)
full_pred_adjusted = np.array([clf.classes_[np.argmax(row)] for row in full_probs])

for class_name, threshold in thresholds.items():
    class_index = class_indices[class_name]
    high_prob_indices = (full_probs[:, class_index] >= threshold)
    full_pred_adjusted[high_prob_indices] = class_name

new_df['model_category_adjsut'] = full_pred_adjusted
...
for key, scorer in scoring.items():
    cv_scores = cross_val_score(clf, features, target, scoring=scorer, cv=skf)
    scores[key] = np.mean(cv_scores), np.std(cv_scores)
    
for metric, (avg, std) in scores.items():
